...
from utils import get_mean_reconstruct_sc, label_to_int, identify_anchors, mmd_rbf, perceptual_loss
...
def get_mlp(adata_l, tag_l, section_ids, hidden_features, out_features, theta, lr, label_name, class_rep, gamma):
    ...
    label_predictor_sc = LabelPredictor_sc(hidden_features, out_features,
                                           class_features=len(set(list(label_sc.numpy()))))
    ...
    for epoch in range(200):
        ...
        for i in range(n):
            if tag_l[i] != 'single cell':
                ...
                z_dists_i = torch.cdist(torch.tensor(adata_l[i].obsm['embedding']),
                                        torch.tensor(adata_l[i].obsm['embedding']), p=2)
        ...
        if epoch % 10 == 0:
            print(
                'epoch {:>3d}: overall loss: {:6.4f},sc classifier loss: {:6.4f},representation loss: {:6.4f},within spatial regularization loss: {:6.4f}'.format(
                    epoch, loss_sc_all, loss_sc, loss_rec, loss_reg_st_within))
    ...
    with torch.no_grad():
        ...
        for i in range(n):
            if tag_l[i] != 'single cell':
                ...
                mlp_m = pd.DataFrame(class_prob_sc_i.cpu().detach().numpy().T, index=label_list,
                                     columns=adata_l[i].obs.index)  # classifier order
...
def MuSTHSC(adata_l, tag_l, section_ids, multiscale, n_clusters_l, in_features, feature_method, hidden_features,
          out_features, iter_comb, anchors_all, use_rep_anchor, alpha, beta, lamb, theta, gamma, delta, edge_weights,
          add_topology, add_feature, add_image, add_sc, spatial_regularization_strength,
          lr=1e-3, subepochs=200, epochs=400, class_rep=None, label_name='Ground Truth', anchor_size=5000, agg_neigh=10,
          mnn_neigh=30, random_seed=1, gpu=0, regularization_acceleration=True, edge_subset_sz=1000000):
    ...
    for i in range(n):
        ...
        if add_feature and 'adj_f' in adata_l[i].obsp.keys():
            adj_f_l.append(
                torch.from_numpy(np.multiply(adata_l[i].obsp['adj_f'].todense().astype("float32"), edge_weights[1])))
            ...
            if tag_l[i] == 'single cell' and add_feature == False:
                adj_f_l.append(torch.from_numpy(
                    np.multiply(adata_l[i].obsp['adj_f'].todense().astype("float32"), edge_weights[1])))
                edge_index_f_l.append(sparse_mx_to_torch_edge_list(sparse.csr_matrix(adata_l[i].obsp['adj_f'])))
            else:
                adj_f_l.append(None)
        ...
        if add_image and 'adj_i' in adata_l[i].obsp.keys():
            if scipy.sparse.issparse(adata_l[i].obsp['adj_i']):
                adj_i_l.append(torch.from_numpy(
                    np.multiply(adata_l[i].obsp['adj_i'].todense().astype("float32"), edge_weights[2])))
            else:
                adj_i_l.append(
                    torch.from_numpy(np.multiply(adata_l[i].obsp['adj_i'].astype("float32"), edge_weights[2])))
    ...
    if feature_method is None or feature_method == 'GCNConv':
        feature_extractor = GraphsEncoder(in_features=in_features, hidden_features=hidden_features,
                                          out_features=out_features)
    if feature_method == 'LGCN_mlp':
        feature_extractor = GraphsEncoder_LGCN_mlp(in_features=in_features, hidden_features=hidden_features,
                                                   out_features=out_features)
    if feature_method == 'SGCN':
        feature_extractor = GraphsEncoder_SGCN(in_features=in_features, hidden_features=hidden_features,
                                               out_features=out_features)
    if feature_method == 'sparse':
        feature_extractor = GraphsEncoder_sparse(in_features=in_features, hidden_features=hidden_features,
                                                 out_features=out_features)
    ...
    for i in range(n):
        if tag_l[i] == 'ST_ref':
            ...
            label_predictor_st_i = LabelPredictor_st(hidden_features, out_features,
                                                     class_features=len(set(list(label_i.numpy()))))
    ...
    for epoch in range(subepochs):
        ...
        for i in range(n):
            if tag_l[i] != 'single cell':
                if feature_method == 'LGCN_mlp':
                    hidden_emb, _, h1_f, h1_t, h1_i, h2, ret = feature_extractor(feat_l[i], feat_a_l[i],
                                                                                 edge_index_f_l[i], adj_f_l[i],
                                                                                 edge_index_t_l[i], adj_t_l[i],
                                                                                 edge_index_i_l[i], adj_i_l[i])

                if feature_method is None or feature_method == 'GCNConv' or feature_method == 'SGCN' or feature_method == 'sparse':
                    hidden_emb, _, h1_f, h1_t, h1_i, h2, ret = feature_extractor(feat_l[i], feat_a_l[i], adj_f_l[i],
                                                                                 adj_t_l[i], adj_i_l[i])
                ...
                ret_l.append(ret)
                if h1_t is not None:
                    ret_CSL = torch.from_numpy(
                        np.concatenate([np.ones([feat_l[i].shape[0], 1]), np.zeros([feat_l[i].shape[0], 1])], axis=1))
                    loss_CSL = nn.BCEWithLogitsLoss()
                    loss_sl = loss_CSL(ret, ret_CSL)
                else:
                    loss_sl = 0

                if adj_i_l[i] is not None:
                    raw = adj_i_l[i]
                    if isinstance(raw, torch.Tensor):
                        A_img = raw.to(h2.device)
                    else:
                        A_img = torch.from_numpy(raw.toarray()).to(h2.device)
                    A_pred = torch.matmul(h2, h2.T)
                    loss_img = perceptual_loss(A_pred, A_img)
                else:
                    loss_img = torch.tensor(0., device=h2.device)

                loss_feat = F.mse_loss(feat_l[i], h2)
                loss_feat_i = loss_feat
                loss_img_i = loss_img
                loss_i1 = loss_feat_i + delta * loss_img_i + alpha * loss_sl

                if spatial_coords_l[i] is not None:
                    if regularization_acceleration:
                        nodes = feat_l[i].shape[0]
                        cell_random_subset_1, cell_random_subset_2 = torch.randint(0, nodes,
                                                                                   (edge_subset_sz,)), torch.randint(0,
                                                                                                                     nodes,
                                                                                                                     (
                                                                                                                     edge_subset_sz,))
                        z1, z2 = torch.index_select(h1_t_l[i], 0, cell_random_subset_1), torch.index_select(h1_t_l[i],
                                                                                                            0,
                                                                                                            cell_random_subset_2)
                        c1, c2 = torch.index_select(spatial_coords_l[i], 0, cell_random_subset_1), torch.index_select(
                            spatial_coords_l[i], 0, cell_random_subset_2)
        ...
        if epoch % 10 == 0:
            print('epoch {:>3d}: train spatial C loss: {:6.4f}, train F loss: {:6.4f},'.format(epoch, running_C_st_loss,
                                                                                               running_F_loss))

    with torch.no_grad():
        for i in range(n):
            if feature_method == 'LGCN_mlp':
                hidden_emb, att, h1_f, h1_t, h1_i, h2, ret = feature_extractor(feat_l[i], feat_a_l[i],
                                                                               edge_index_f_l[i], adj_f_l[i],
                                                                               edge_index_t_l[i], adj_t_l[i],
                                                                               edge_index_i_l[i], adj_i_l[i])

            if feature_method is None or feature_method == 'GCNConv' or feature_method == 'SGCN' or feature_method == 'sparse':
                hidden_emb, att, h1_f, h1_t, h1_i, h2, ret = feature_extractor(feat_l[i], feat_a_l[i], adj_f_l[i],
                                                                               adj_t_l[i], adj_i_l[i])
    ...
    if len(ref_sc_id) > 0 and multiscale:
        ...
        adata_l = get_mlp(adata_l, tag_l, section_ids, hidden_features, out_features, theta, lr, label_name, class_rep,
                          gamma)
    ...
    for epoch in range(subepochs, epochs):
        ...
        epoch_losses.append(loss_F.item())
        
        if epoch % 10 == 0:
            print(
                'epoch {:>3d}: total loss:{:6.4f}, train F loss: {:6.4f}, train C loss: {:6.4f}, train D loss: {:6.4f}'.format(
                    epoch, loss_F_all, running_F_loss, running_C_st_loss, running_D_loss))
    flag = 0
    ...
    with torch.no_grad():
        ...
        for i in range(n):
            ...
            if feature_method == 'LGCN_mlp':
                hidden_emb, att, h1_f, h1_t, h1_i, h2, _ = feature_extractor(feat_l[i], feat_a_i, edge_index_f_l[i],
                                                                             adj_f_l[i], edge_index_t_l[i], adj_t_l[i],
                                                                             edge_index_i_l[i], adj_i_l[i])

            if feature_method is None or feature_method == 'GCNConv' or feature_method == 'SGCN' or feature_method == 'sparse':
                hidden_emb, att, h1_f, h1_t, h1_i, h2, _ = feature_extractor(feat_l[i], feat_a_i, adj_f_l[i],
                                                                             adj_t_l[i], adj_i_l[i])
    ...
    # Visualization: plot loss over time
    plt.plot(range(subepochs), epoch_losses, label="Training Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
